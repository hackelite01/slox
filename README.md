...
|slox|Multi Vulnerability Scanner|for web application|
|----------------|--------------|-------------|
...
> **slox** is an easy-to-use tool that finds web issues like `LFI` - `OR` - `SQLi` - `XSS` - `CRLF`. <br>
...
```bash
git clone https://github.com/hackelite01/slox.git
```
```bash
cd slox
```
...
> [!WARNING]  
> slox is intended for educational and ethical hacking purposes only. It should only be used to test systems you own or have explicit permission to test. Unauthorized use of third-party websites or systems without consent is illegal and unethical.
